 ...
 * [**Topic 3:**](https://jeremymack-lu.github.io/shinyapps/#24) Reactive Programming (server)
 
 * [**Topic 4:**](https://jeremymack-lu.github.io/shinyapps/#35) Layouts and Themes
 
 * [**Topic 5:**](https://jeremymack-lu.github.io/shinyapps/#48) Example - Lehigh Valley Temperature
 
 * [**Topic 6:**](https://jeremymack-lu.github.io/shinyapps/#57) Deploying Shiny Apps
 
 * [**Topic 7:**](https://jeremymack-lu.github.io/shinyapps/#60) Resources
...
* Error messages can be a bit more cryptic than regular R error messages.
...
ui <- dashboardPage(skin="black",
                    dashboardHeader(
                      title="Lehigh Valley Temperature", titleWidth=350),
                    dashboardSidebar(
                      width=350,
                      selectInput(
                        inputId="years",
                        label="Select year(s):",
                        choices=seq(1948,2021,1),
                        multiple=TRUE)),
                    dashboardBody(
                      fluidRow(plotOutput("plot", height='500px')),
                      br(),
...
server <- function(input, output, session) {
  ...
  output$data <- renderTable(
    if(is.null(input$years)) paste("To plot data, please select a year")
    else df.sub() %>% mutate(month=as.integer(month),label=month.abb[month],Year=as.integer(year)) %>%
      pivot_wider(id_cols=Year,names_from=label,values_from=avg) %>%
      mutate(Annual_mean=rowMeans(.[,2:13], na.rm=TRUE)) %>%
      mutate(Annual_diff=Annual_mean-mean(df$dmt, na.rm=TRUE)) %>%
      mutate(Year=as.character(Year)) %>%
      bind_rows(monthly.sum.W) %>%
      rename('Annual Mean'=Annual_mean, 'Annual Diff'=Annual_diff))
```
]
...
server <- function(input, output, session) {
  ...
  output$map <- renderLeaflet(
    leaflet() %>% setView(lat=map$lat, lng=map$lng, zoom=map$zoom) %>%
